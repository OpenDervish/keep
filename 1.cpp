...
void polygon(int a,int b,int c,int d,int e)			
{
	...
	glColor3f(1.0,1.0,1.0);
   	glPointSize(3.0);			//dots on dice face
        ...
        {
            case 1 :glVertex3f(1200.0,600.0,-50.0);
					break;
            case 2 :    glVertex3f(1175.0,651.0,25.0);
...
void pawns::check()
{
	...
	for(i=0;i<4;i++)
	{
		if(a==l[i].xp&&b==l[i].yp)			//check ladder matching
		{
			...
			m=(l[i].yp*70.0+y2+oy-(l[i].yq*70.0+y2+oy))/(l[i].xp*70.0+x0+30-(l[i].xq*70.0+x0+30));			//move up that ladder
...
void display2()
{
    ...
    glPushMatrix();
		...
		glRotatef(theta[0],1.0,1.0,0.0);			//x-y
